...
class SlideInTransition {
  const SlideInTransition();

  Widget call(BuildContext context, Widget child) => TweenAnimationBuilder<Offset>(
        tween: Tween(
          begin: Offset(1, 0),
          end: Offset(0, 0),
        ),
...
class PeopleScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Wouter(
        base: '/people',
        child: WouterSwitch(
          transition: const SlideInTransition(),
          routes: {
          ...
          children: people.values
              .map(
                (person) => ListTile(
                  ...
                  onTap: () => context.wouter.push('/people/${person['_id']}'),
                ),
              )
              .toList(),
        ),
